...
impl Actuator {
    pub fn new(props: ActuatorProps) -> Actuator {
        ...
        let contract_valve = DcMotor::try_new(&mut contract_pwm, props.contract_motor).unwrap();
        let expand_valve = DcMotor::try_new(&mut expand_pwm, props.expand_motor).unwrap();
